  ...
  const plotData = [
    {
      ...
      name: 'P<sub>true</sub>',
      line: { 
        color: `rgba(76, ${Math.min(255, Math.floor(175 + (80/15) * trialNum))}, ${Math.min(255, Math.floor(80 + (175/15) * trialNum))}, 0.2)`
      },
    },
    {
      x: x_values,
      ...
      line: { 
        color: `rgba(255, ${Math.min(255, Math.floor(82 + (173/15) * trialNum))}, 82, 0.2)`
      },
    }
  ];
...
const AverageTrajectories = () => {
  ...
  for (let i = 0; i < totalTrials*2; i++) {
    if (i % 2 === 0) {
      // these are P_true plots
      let plotTrace = plotData[i]; 
      let sampleTrajectory = plotTrace.y;
      for (let j = 0; j < n; j++) {
        ptrue_avg_y[j].push(Math.log(sampleTrajectory[j]))
      };
    } else {
      // these are P_guess plots
      let plotTrace = plotData[i]; 
      let sampleTrajectory = plotTrace.y;
      for (let j = 0; j < n; j++) {
        pguess_avg_y[j].push(Math.log(sampleTrajectory[j]))
      };
    };
  };

  ptrue_avg_y = ptrue_avg_y.map((listOfLogProbs) => {
      let logAvg = listOfLogProbs.reduce((a, b) => a + b, 0) / listOfLogProbs.length;
      return Math.exp(logAvg);
    }
  );

  pguess_avg_y = pguess_avg_y.map((listOfLogProbs) => {
      let logAvg = listOfLogProbs.reduce((a, b) => a + b, 0) / listOfLogProbs.length;
      return Math.exp(logAvg);
    }
  );
