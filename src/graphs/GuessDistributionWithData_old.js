...
const GuessDistributionWithData = () => {
  ...
  const totalGuessLikelihood = guessProbabilities.reduce((total, curr) => total * curr, 1);
  const totalGuessLikelihoodString = totalGuessLikelihood.toExponential(2).replace('e-', ' x 10<sup>-') + '</sup>';
  ...
  const layout = {
    ...
    annotations: [
      {
        text: 'total likelihood = ' + totalGuessLikelihoodString,
        xref: 'paper',
        yref: 'paper',
        x: 1,
        y: 0.5,
